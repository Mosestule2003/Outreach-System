...
const FOOTER_LINKS = [
  ...
  },
  {
    heading: 'Get started',
    links: [
      { label: 'Book a demo', href: 'https://cal.com/rezlv-official/15min' },
    ...
    <footer className="border-t border-border px-4 py-16">
      <div className="mx-auto max-w-6xl">
        <div className="grid gap-10 md:grid-cols-[1.5fr_repeat(3,1fr)]">
          <div>
            ...
            <p className="mt-4 max-w-xs text-sm leading-relaxed text-muted-foreground">
              Exception resolution orchestration for Canadian Shopify DTC brands. Detect, contact,
              prepare the fix. Your team just confirms.
            </p>
          ...
          {FOOTER_LINKS.map((col) => (
            <div key={col.heading}>
              <h3 className="font-mono text-[11px] font-medium uppercase tracking-[0.14em] text-foreground">{col.heading}</h3>
                ...
                {col.links.map((link) => (
                  <li key={link.label}>
                    {link.label === 'Get early access' ? (
                      ...
                      <a
                        href={link.href}
                        target={link.label === 'Book a demo' ? '_blank' : undefined}
                        rel={link.label === 'Book a demo' ? 'noopener noreferrer' : undefined}
                        onClick={() => {
        ...
        <div className="flex flex-col items-start justify-between gap-4 sm:flex-row sm:items-center">
          ...
          <p className="text-xs text-muted-foreground">
            Everybody notifies. Rezlv resolves.
          </p>
